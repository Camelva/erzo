...
func (v *Video) parseVideoInfo(info string) error {
	...
	var date string
	if prData.Microformat.PlayerMicroformatRenderer.PublishDate != "" {
		date = prData.Microformat.PlayerMicroformatRenderer.PublishDate
	} else if prData.Microformat.PlayerMicroformatRenderer.UploadDate != "" {
		date = prData.Microformat.PlayerMicroformatRenderer.UploadDate
	} else {
		date = time.Now().Format("2006-01-02")
	}
	v.PublishDate, _ = time.Parse("2006-01-02", date)
